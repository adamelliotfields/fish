function google -d 'Open Google pages from the terminal.'
  ...
  # help
  set -q _flag_h && begin
    echo 'google: Open Google pages from the terminal.'
    echo
    echo (set_color -o)'USAGE'(set_color normal)
    echo '  google [flags] [--] [query]'
    echo
    ...
    echo '  $ google calculator'
    echo '  $ google color picker'
    echo '  $ google tic tac toe'
    echo '  $ google do a barrel roll'
    echo '  $ google google doodle'
    echo '  $ google "i\'m feeling curious"'
    return 0
  end

  # check for open
  if not command -q open
    echo 'google: open is required'
    return 1
  end
