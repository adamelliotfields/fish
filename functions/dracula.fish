function dracula -d 'Dracula Pro theme switcher for Hyper terminal'
  set -l args \
    'h/help' \
    'u/uninstall'
  ...
  if test -z $theme
    echo 'dracula: no theme specified'
    return 1
  end

  if not contains $theme $themes
    echo 'dracula: unknown theme "'$theme'"'
    return 1
  end
  ...
  if not test -e $src_path'/'$theme_long
    echo 'dracula: theme "'$theme'" not downloaded'
    return 1
  end
